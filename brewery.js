// Description
// Today, we want to replicate the basic functionalities of the BreweryDB API we used last week. If yesterday we had to rely on someone else to have a backend, now let's build our own! No more json-server!
// @here
// Instructions
...
// Extra
// In real-world applications, managing edge cases is important. What would be the most appropriate response if:
// - The :id entered does not exist
// - The TYPE entered is not between those "valid"
// There is not a 100% right||100% wrong answer here, make your guesses and we will review them tomorrow.

// Tips
// - Start with small steps: setup a nodejs server with a simple route and add additional routes only when things are working as you expect
// - Use nodemon
// - We are just using GET endpoints so you can use your browser to test your API
...
// (req,res)---> CALLBACKS
// breweries?brewery_type=micro
app.get("/breweries", (req, res) => {
  const brewerieType= req.query.brewery_type
  let filteredBreweries = breweriesObj.filter(
    (brewery)=>brewery.brewery_type === brewerieType
  )

  const response = filteredBreweries.length ? filteredBreweries : "We can't find any brewery"

  if(brewerieType){
  res.json({response})
  } else {
  res.json({breweries: breweriesObj, timestamp: Date.now()});
  }
});
...
app.get("/breweries/:id", (req, res) => {
    ...
    res.json({brewery})
        ...
        "Sorry, we couldn't find any brewery!"
    }
  });
